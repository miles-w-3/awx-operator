...
helm-index:
	...
	for tag in $(TAGS); do\
		dl_url="https://github.com/ansible/$${CHART_REPO}/releases/download/$${tag}/$${CHART_REPO}-$${tag}.tgz";\
		curl -RLOs -z "$${CHART_REPO}-$${tag}.tgz" --fail $${dl_url};\
		result=$$?;\
		if [ $${result} -eq 0 ]; then\
			echo "Downloaded $$dl_url";\
		else\
			echo "Skipping release $${tag}; No helm chart released";\
	...
	@cd ..
	#$(HELM) repo index .cr-release-packages/ --url https://$${CHART_OWNER}.github.io/awx-operator/ --merge index.yaml
	$(HELM) repo index . --url https://ansible.github.io/awx-operator/ --merge index.yaml
